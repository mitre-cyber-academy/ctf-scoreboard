...
class UserRequestTest < ActiveSupport::TestCase
  ...
  test 'uniqueness of pending request' do
    create(:user_request, team: @team, user: @user)
    user_request = build(:user_request, team: @team, user: @user)
  ...
  test 'user on a team' do
    user_request = create(:user_request, team: @team, user: @user)
  ...
  test 'accept request from user on a team' do
    ...
    user_request = create(:user_request, team: @team, user: non_captain)
    assert_no_difference '@team.users.reload.size' do
      user_request.accept
    end
  end
