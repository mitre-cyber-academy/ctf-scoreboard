...
class TeamDisplayModesTest < ActionDispatch::IntegrationTest
  ...
  test 'team creation page' do
    ...
    assert_select 'h1', /Create a Team/, 'Create a team should show on the team creation page'
    assert_select 'form[id=new_team]' do
      assert_select 'label:nth-child(1)', 'Team name', 'Create team table should display correctly'
      assert_select 'label:nth-child(1)', 'Affiliation', 'Create team table should display correctly'
      assert_select 'label:nth-child(1)', 'Division', 'Create team table should display correctly'
    end
  end

  test 'join team page' do
    sign_in create(:user)
    get "/users/join_team"
    ...
    assert_select 'body' do
      assert_select 'div[class=container]' do
        assert_select 'div:nth-child(1)' do
          assert_select 'div:nth-child(1)' do
            assert_select 'h4', I18n.t('teams.show.team_eligibility_status')
          end
  ...
  test 'solved challenges show on team summary page' do
    ...
    @team = create(:team)
    solved_challenge = create(:standard_solved_challenge, team: @team, challenge: challenge1)
    ...
    assert_select 'table[class=table\ table-bordered\ table-striped]' do
      assert_select 'thead' do
        assert_select 'tr' do
          ...
          assert_select 'th:nth-child(2)', I18n.t('teams.summary.solved_challenges_table.challenge_header')
          assert_select 'th:nth-child(3)', I18n.t('teams.summary.solved_challenges_table.points_header')
          assert_select 'th:nth-child(4)', I18n.t('teams.summary.solved_challenges_table.when_header')
        end
      end
      assert_select 'tbody' do
        assert_select 'tr' do
          ...
          assert_select 'td:nth-child(2)' do
            assert_select 'a[href=\/game\/challenges\/' + challenge1.id.to_s + ']'
          end
          assert_select 'td:nth-child(3)', '100'
          assert_select 'td:nth-child(4)', solved_challenge.created_at.strftime('%B %e at %l:%M %p %Z')
        end
      end
    end
  end
