...
def destory
  ...
  def user_params # Be sure to update this in the confirmations controller as well if you need to add parameters.
    params.require(:user).permit(:username, :password, :password_confirmation, :remember_me, :team_id, :team_captain, :email, :name, :school, :year_in_school, :gender, :compete_for_prizes ,:age, :area_of_study, :location, :state, :resume, :verification)
  end
end
